...
export default function PngToSvg({ onClose }) {
  ...
  const convertToSvg = async () => {
    ...
    try {
      ...
      const convertedBlob = new Blob([selectedFile], { type: 'image/svg+xml' });
      const convertedUrl = URL.createObjectURL(convertedBlob);
      
      setConvertedFile({
        url: convertedUrl,
        name: selectedFile.name.replace('.png', '.svg'),
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-800">PNG إلى SVG</h2>
              <p className="text-gray-600 text-sm">تحويل الصور النقطية إلى متجهات SVG</p>
            </div>
          </div>
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-50 rounded-2xl p-6">
              ...
              >
                {isConverting ? 'جاري التحويل...' : 'تحويل إلى SVG'}
              </button>
            </div>
              ...
              {convertedFile ? (
                <div className="space-y-4">
                  ...
                  <div className="bg-blue-50 rounded-2xl p-4">
                    <p className="text-blue-700 text-sm text-center">
                      🎉 تم تحويل الصورة إلى متجه SVG قابل للتطوير بدون فقدان الجودة!
                    </p>
                  </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 text-sm">
                    {selectedFile ? 'انقر على تحويل لبدء العملية' : 'سيظهر ملف SVG المحول هنا'}
                  </p>
                </div>
              )}
            </div>
